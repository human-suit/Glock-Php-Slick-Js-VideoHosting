...
$resultss = mysqli_query($mysql, "SELECT * FROM `video_host` WHERE `name_video` LIKE '%$filt%'" );
$resultsss = mysqli_query($mysql, "SELECT * FROM `video_host` WHERE `name_video` LIKE '%$filt%'" );
$resultssss = mysqli_query($mysql, "SELECT * FROM `video_host` WHERE `name_video` LIKE '%$filt%'" );
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<link rel="stylesheet" href="css/filter.css" id="theme">
		...
		<div class="video_history">
   		<?php $hum=0; while ($videos = mysqli_fetch_assoc($resultss)) {
			$hum++;?>
				...
				<button id="<?php echo $hum;?>" class="perehod2"></button>
				<div class="width">
				<video height="300" controls="controls" id="vid<?php echo $hum;?>" class="video_med" src="donloads/musics/<?php echo $videos['url_video'];?>" ></video>
                </div>
            <?php }?>
		</div>
		</div>
